...
| `record.encodedRecord` and `record.coder` | The same element [encoded with its coder](/asgarde/concepts/failure/#encoded-elements) with `withEncodedElements` (since 1.4.0), empty otherwise |
